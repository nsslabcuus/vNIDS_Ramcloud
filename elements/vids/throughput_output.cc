...
Throughput::Throughput(): _timer(this), pkt_cnts(0),
                          pkt_sizes(0), allpkt_cnts(0),
                          allpkt_sizes(0), output_interval(1),pos(0)
{}
...
int Throughput::configure(Vector<String> &conf, ErrorHandler *errh)
{
    ...
    return Args(conf, this, errh).read("interval", output_interval)
           .read("pos", pos)
           .execute();
}
...
void Throughput::update()
{
    ...
    printf("Postion %d, Packets Recived %d, Packets Sizes %d, All cnts %d, All sizes %d\n",

 ...
 */
int geneve_opt_len = 0;

/*
 * Let the element be able to configure the dest mac address for the geneve header
 */
 ...
 */
Packet *
GeneveEncap::simple_action(Packet *p_in)
{
    uint32_t size = 14 + sizeof(click_udp) + sizeof(click_ip) + sizeof(click_geneve) + _opt_len*4;
    WritablePacket *p = p_in->push(size);
    // copy the ether header. @todo may need to fill the ip header
    memcpy(p->data(), p->data() + size, 14);
    click_geneve *gh = (click_geneve*)(p->data() + 14 + sizeof(click_ip) + sizeof(click_udp));
    gh->opt_len = _opt_len;

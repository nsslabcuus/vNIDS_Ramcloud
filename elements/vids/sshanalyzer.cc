...
void SSHAnalyzer::push(int port, Packet* p)
{
    ...
    (void)port;
    const unsigned char* payload = p->transport_header() + (p->tcp_header()->th_off << 2);
    int payload_size = p->end_data() - payload;
    UNUSED(payload_size);
    // if the server replies the client the ssh protocol version
    if(/*payload_size >= version_size
            &&*/ ntohs(p->tcp_header()->th_sport) == 22)//&& 0 == strncmp((const char*)payload, ssh_version, version_size)) //
    {

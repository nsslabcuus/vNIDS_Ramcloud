 ...
 */
Packet* TCPClassifier::reassemble(Packet *p) {
    return p;
}
...
void TCPClassifier::classify(Packet* p) {
    ...
    uint16_t th_sport = ntohs(tcp->th_sport);
    uint16_t th_dport = ntohs(tcp->th_dport);
    // @note Assume that the smaller port indicates the protocol
    uint16_t small_port = th_sport <= th_dport ? th_sport : th_dport;

    // LOGE("port %d -> %d : len %u", th_sport, th_dport, p->length());

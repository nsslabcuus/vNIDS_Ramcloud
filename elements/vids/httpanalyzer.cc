...
#define BINPAC_HTTP_PARSER
    ...
    {
        error_flag = 1;
        LOGE("Binpac exception: HTTP parse failed, Binpac exception: %s", e.c_msg());
    }
    ...
    if (error_flag == 0 && headers.size() > 0)
    {
        ...
        if (content_type)
        {
            ...
            if (strncmp(content_type.c_str(), "text/html", 9) == 0)
            {
                ...
                p->kill();
                     ...
                     strncmp(content_type.c_str(), "application/x-msdos-program", 27) == 0)
            {
                event_t *event = alloc_event_data(0);
                event->event_type = HTTP_RESPONSE_EXE;
                event->fill_connect(p);
        ...
        if (cookie)
        {
            ...
            event->fill_connect(p);
            event->get_writer()(cookie.length(), cookie.c_str())(useragent.length(), useragent.c_str());
